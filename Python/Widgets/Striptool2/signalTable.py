...
class customPVFunction(QObject):
    def __init__(self, parent = None, pv=None):
        super(customPVFunction, self).__init__(parent)
        self.pv = PVBuffer(pv)

    def getValue(self):
        return float(self.pv.lastValue())
...
class signalTable(QWidget):
    ...
    def __init__(self, parent = None, VELAMagnetController=None, CLARAMagnetController=None, BPMController=None, GeneralController=None):
        ...
        self.stream = open(os.path.abspath(pathname) + '/settings.yaml', 'r')
        ...
        self.customSelectionBox.combo1.setCurrentIndex(-1)
    ...
    def addRow(self, name, functionForm, functionArgument, freq, colourpickercolour, logScale=False, **kwargs):
        if functionForm == 'custom':
            testFunction = customPVFunction(parent=self, pv=functionArgument).getValue
        else:
    ...
    def addTableRowCustom(self):
        ...
        self.addRow(name.replace("_","$"), functionForm, name, freq, colourpickercolour, logScale=logScale)
