...
class multiKnob_GroupBox(QGroupBox):
    ...
    def __init__(self, parent = None):
        ...
        for c,h in enumerate(['On', 'PV Name','Multiplier','Current Value', 'Original Value']):
            label = QLabel(h)
...
class combinedKnob_GroupBox(QGroupBox):
    ...
    knobChanged = pyqtSignal('float', 'float')
    ...
    def __init__(self, parent = None):
        ...
        self.multiplierDisplay = QLineEdit("{0:.3f}".format(1))
        ...
        self.multiplierDisplay.setMaximumHeight(30)


        self.controlWidget = QWidget()
        self.controlWidgetLayout = QVBoxLayout()
        self.controlWidget.setLayout(self.controlWidgetLayout)
        self.controlDisplayLabel = QLabel('Step Size')
        f = QFont( "Arial", 20, QFont.Bold)
        self.controlDisplayLabel.setFont(f)
        self.controlDisplayLabel.setAlignment(Qt.AlignCenter);
        self.controlDisplayLabel.setMaximumHeight(20)
        ...
        self.display.setMaximumHeight(30)
        self.controlWidgetLayout.addWidget(self.controlDisplayLabel)
        self.controlWidgetLayout.addWidget(self.display)
        ...
        self.frameLayout.addWidget(self.multiplierDisplay)
        self.frameLayout.addWidget(self.controlWidget)
        self.frameLayout.addLayout(self.leftrightLayout)
    ...
    def emitSignal(self, sign):
        self.multiplierDisplay.setText("{0:.3f}".format(1+self.multiplier))
        if abs(sign) > 0:
            self.knobChanged.emit(self.multiplier, sign*self.display.value())
...
class relative_knob(QWidget):
    ...
    def __init__(self, actuator=None, strength=1, on=True):
        ...
        self.spinWidget.valueChanged.connect(self.spinBoxChanged)

        self.valueWidget = QLineEdit()
        self.valueWidget.setReadOnly(True)

        self.originalValueWidget = QLineEdit()
        self.originalValueWidget.setReadOnly(True)
    ...
    def changePV(self, index):
        ...
        self.originalValueWidget.setText("{0:.3f}".format(self.initialValue))
    ...
    def pvEmitted(self, time, value):
        self.spinWidget.blockSignals(True)
        self.spinWidget.setValue(value / self.initialValue)
        self.valueWidget.setText("{0:.4f}".format(value))
        self.spinWidget.blockSignals(False)
    ...
    @property
    def widgets(self):
        return [self.enabledWidget, self.nameWidget, self.spinWidget, self.valueWidget, self.originalValueWidget, self.deleteButton]

    def knobChanged(self, total=0, multiplier=0):
        if self.enabledWidget.isChecked():
            self.spinWidget.setValue(self.spinWidget.value() + multiplier)
        self.pv.setValue(self.initialValue * self.spinWidget.value())
...
class multiknob(QMainWindow):
    def __init__(self, inputfile=None, parent = None):
        ...
        if inputfile is not None:
            ...
            self.knobsFilename = os.path.dirname( os.path.abspath(__file__)) + "/knobs.cmk"
        self.loadSettings(self.knobsFilename)

    def closeTab(self, tab):
        widget = self.widget.widget(tab).multiKnobGroup
        widgetrows = []
        for r in widget.knobs:
            widgetrows.append(r)
        for r in widgetrows:
            widget.deleteRow(r)
    ...
    def saveSettings(self):
        filename, ok = QFileDialog.getSaveFileName(self, 'Save File', QDir.currentPath(),
          "Control files (*.cmk) ;; All files (*.*)")
        if ok:
            fileName, fileExtension = os.path.splitext(filename)
            if fileExtension == '':
                filename = filename + '.cmk'
    ...
    def loadSettings(self, filename=None):
        if filename is None or filename is False:
            self.knobsFilename, ok = QFileDialog.getOpenFileName(self, 'Open File', QDir.currentPath(),
          "Control files (*.cmk) ;; All files (*.*)")
        else:
            ok = True
...
class knobTab(QWidget):
    ...
    def __init__(self, label, parent = None, **kwargs):
        ...
        self.combinedKnobGroup.setMaximumWidth(200)
    ...
    def addKnob(self):
        ...
        self.combinedKnobGroup.resetSignal.connect(knob.reset)
